...
Deno.serve(async (req) => {
  try {
    ...
    if (!headlines) return new Response(JSON.stringify({ message: "No news found" }), { status: 404 })

    // 2. Summarize & Embed with Gemini 1.5 Flash
    const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" })
    const summaryResult = await model.generateContent(`Summarize for ${ticker}: ${headlines}`)
